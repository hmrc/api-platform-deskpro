...
import uk.gov.hmrc.apiplatformdeskpro.domain.models.mongo.UploadStatus.{Failed, FailedUploadToDeskpro, UploadedSuccessfully}
...
class FileAttachmentWarningsSpec extends AsyncHmrcSpec with FixedClock {
  ...
  val attachedFilesWarning      = "<h4 class=\"govuk-heading-s govuk-!-margin-bottom-1\">Attached files</h4>"
  ...
  "addMessageFileUploadWarnings" should {
    "successfully add warnings to the message where an upload has failed in upscan" in {
      ...
      result shouldBe s"${message}${attachedFilesWarning}${fileName}<br><p class='govuk-body govuk-!-margin-bottom-0 govuk-!-margin-top-1 govuk-!-font-size-16'>The file is not one of the accepted file types and has not been received.</p><hr class='govuk-section-break govuk-!-margin-top-2 govuk-!-margin-bottom-3 govuk-section-break--visible'>"
    }
    ...
    "successfully add warnings to the message where an upload to Deskpro has failed" in {
      ...
      result shouldBe s"${message}${attachedFilesWarning}${fileName}<br><p class='govuk-body govuk-!-margin-bottom-0 govuk-!-margin-top-1 govuk-!-font-size-16'>The file has passed virus scanning but has failed to upload.</p><hr class='govuk-section-break govuk-!-margin-top-2 govuk-!-margin-bottom-3 govuk-section-break--visible'>"
    }
    ...
    "successfully add warnings to the message where a file has not been uploaded to upscan yet" in {
      ...
      result shouldBe s"${message}${attachedFilesWarning}${fileName}<br><p class='govuk-body govuk-!-margin-bottom-0 govuk-!-margin-top-1 govuk-!-font-size-16'>The file is in a queue to be scanned for viruses.</p><hr class='govuk-section-break govuk-!-margin-top-2 govuk-!-margin-bottom-3 govuk-section-break--visible'>"
    }
  }
  ...
  "amendMessageFileAttachmentWarnings" should {
    "successfully remove warnings from the message where an upload which had previously not been uploaded to upscan has now succeeded" in {
      val messageWithWarnings =
        s"${message}${attachedFilesWarning}${fileName}<br><p class='govuk-body govuk-!-margin-bottom-0 govuk-!-margin-top-1 govuk-!-font-size-16'>The file is in a queue to be scanned for viruses.</p><hr class='govuk-section-break govuk-!-margin-top-2 govuk-!-margin-bottom-3 govuk-section-break--visible'>"
