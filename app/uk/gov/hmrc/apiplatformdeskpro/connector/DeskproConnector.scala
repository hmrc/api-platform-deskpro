...
class DeskproConnector @Inject() (http: HttpClientV2, config: AppConfig, metrics: ConnectorMetrics, val clock: Clock)(implicit val ec: ExecutionContext)
  ...
  def createBlob(fileName: String, fileType: String, src: Source[ByteString, _])(implicit hc: HeaderCarrier): Future[DeskproBlobWrapperResponse] = metrics.record(api) {
    ...
    metrics.record(api) {
      http
        ...
        .execute[DeskproBlobWrapperResponse]
    }
  }
